...
export function OutsideTankerLog({
  ...
  const blank = useMemo(
    () => ({ slot: slotNow(), kind: 'purchase', product_id: '', party_id: '', tankers: '', note: '' }),
    []
  )
  ...
  const parties = form.kind === 'sales' ? customers : suppliers
  ...
  async function add(): Promise<void> {
    ...
    try {
      ...
      setForm({ ...blank, slot: form.slot, kind: form.kind })
      await load()
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-4xl">
          ...
          <div className="flex flex-wrap items-end gap-3 rounded-[4px] border border-[#D6E2D6] bg-[#F7FAF6] p-3">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Select value={form.slot} onValueChange={(v) => setForm((p) => ({ ...p, slot: v }))}>
                <SelectTrigger className="h-9 w-[110px] text-[12.5px] font-bold"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Select
                value={form.kind}
                onValueChange={(v) => setForm((p) => ({ ...p, kind: v, party_id: '' }))}
              >
                <SelectTrigger className="h-9 w-[190px] text-[12.5px] font-bold"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="flex flex-col gap-1.5">
              <Label className="text-[10px] font-extrabold uppercase tracking-[.13em] text-[#5A6B62]">Product</Label>
              <Select value={String(form.product_id)} onValueChange={(v) => setForm((p) => ({ ...p, product_id: v }))}>
                <SelectTrigger className="h-9 w-[170px] text-[12.5px] font-bold"><SelectValue placeholder="Any product" /></SelectTrigger>
                <SelectContent>
                  {products.map((pr) => (
                    <SelectItem key={String(pr.id)} value={String(pr.id)}>{String(pr.name)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div className="flex flex-col gap-1.5">
              ...
              <Select value={String(form.party_id)} onValueChange={(v) => setForm((p) => ({ ...p, party_id: v }))}>
                <SelectTrigger className="h-9 w-[190px] text-[12.5px] font-bold"><SelectValue placeholder="Any party" /></SelectTrigger>
                <SelectContent>
                  {parties.map((pa) => (
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Input
                ...
                className="h-9 w-[90px] text-right text-[12.5px] font-bold"
                placeholder="0"
              />
            </div>
            <Button
              onClick={() => void add()}
              disabled={saving}
              className="!h-9 !gap-1.5 !rounded-[4px] !bg-[#C7F03F] !px-4 !text-[12.5px] !font-extrabold !text-[#0B3D2E] hover:!bg-[#B9E52C]"
            >
                  ...
                  <TableBody>
                    {s.rows.map((r, i) => (
                      <TableRow key={String(r.id)}>
                        ...
                        <TableCell className="font-bold text-[#0A1F17]">{String(r.product_name || '—')}</TableCell>
